...
interface RecetaData {
    ...
    readonly imagenFlotante: string; // <-- Nueva propiedad para la imagen que aparece al hacer scroll
...
const database: Record<string, RecetaData> = {
    zapallo: {
        ...
        imagenFlotante: '/src/assets/hallow.png', // <-- Cambia esto por tu PNG sin fondo de zapallo
        color: '#C05A2A',
        ingredientes: [
    ...
    caqui: {
        ...
        imagenFlotante: '/src/assets/caqui1.png', // <-- Cambia esto por tu PNG sin fondo de caqui
        color: '#E58A1F',
        ingredientes: [
    ...
    camote: {
        ...
        imagenFlotante: '/src/assets/camote.png', // <-- Cambia esto por tu PNG sin fondo de camote
        color: '#9E4723',
        ingredientes: [
    ...
    guayaba: {
        ...
        imagenFlotante: '/src/assets/guayaba1.png', // <-- Cambia esto por tu PNG sin fondo de guayaba
        color: '#E15A75',
        ingredientes: [
...
function initReceta() {
    // 1. Leer el parámetro de la URL (ej. ?sabor=zapallo)
    const urlParams = new URLSearchParams(window.location.search);
    const sabor = urlParams.get('sabor');

    // Si no hay sabor o no existe en la base de datos, volvemos al inicio
    if (!sabor || !database[sabor]) {
        window.location.href = '/';
        return;
    }
    const btnVolver = document.getElementById('btn-volver') as HTMLAnchorElement | null;
    if (btnVolver) {
    ...
    // 2. Inyectar datos principales en el DOM
    ...
    const imgEl = document.getElementById('img-receta') as HTMLImageElement;
    if (imgEl) {
        imgEl.src = data.imagen;
        imgEl.classList.remove('opacity-0'); // Efecto fade-in
    }

    const beneficiosEl = document.getElementById('beneficios-receta');
    if (beneficiosEl) beneficiosEl.innerHTML = `<p class="text-slate-600 leading-relaxed">${data.beneficios}</p>`;

    // Renderizar Listas
    const ingredientesList = document.getElementById('lista-ingredientes');
    if (ingredientesList) {
        ingredientesList.innerHTML = data.ingredientes.map(ing => 
            `<li class="flex items-start gap-3">
                <svg class="w-6 h-6 shrink-0" style="color: ${data.color}" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"></path></svg>
                <span class="text-slate-700">${ing}</span>
            </li>`
        ).join('');
    }
    ...
    if (pasosList) {
        pasosList.innerHTML = data.pasos.map((paso, index) => 
            `<li class="relative pl-10">
                <span class="absolute left-0 top-1 w-8 h-8 rounded-full flex items-center justify-center text-white font-bold text-sm shadow-md" style="background-color: ${data.color}">${index + 1}</span>
                <p class="text-slate-700 leading-relaxed pt-1">${paso}</p>
            </li>`
        ).join('');
    }

    // 3. Inyectar imagen flotante y arrancar el Intersection Observer
    const imgFlotanteEl = document.getElementById('imagen-flotante') as HTMLImageElement | null;
    const recetaSeccion = document.getElementById('receta-seccion') as HTMLElement | null;

    if (imgFlotanteEl && recetaSeccion) {
        // Asignamos la ruta de la imagen dependiendo del sabor
        ...
        const observerOptions: IntersectionObserverInit = {
            root: null,
            rootMargin: '0px',
            threshold: 0.15 // 15% de la sección debe ser visible para que aparezca
        };

        const scrollObserver = new IntersectionObserver((entries) => {
            entries.forEach(entry => {
                if (entry.isIntersecting) {
                    // El usuario llegó a la receta: Mostramos la fruta
                    imgFlotanteEl.classList.remove('opacity-0', 'translate-y-12');
                    imgFlotanteEl.classList.add('opacity-100', 'translate-y-0');
                } else {
                    // El usuario subió o bajó de más: Ocultamos la fruta
